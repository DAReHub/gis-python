...
def ptServices_details(events_file):
	...
	for child in root:
		...
		if ((type_event == 'PersonLeavesVehicle')):
			...
			temp_arrival_list.append(person_id)
	...
	df_ptArrival.columns =['vehicle_id', 'person_id', 'arrTime']
...
def export_df_to_csv(df, output_dir, scenario_name):
	...
	suffix = '.csv'
	file_name = 'df_ptServices_baseline_vs_' + scenario_name + '_passengersEgress'
...
def main(baseline_events_filepath: str, scenario_events_filepath:str, scenario_name:str, output_dir_filepath: str):
	...
	df_ptServices_comp = df_baseline_ptServices.merge(df_scenario_ptServices, on=['vehicle_id', 'transitLine_id', 'transitRoute_id', 'person_id'], how='outer')
